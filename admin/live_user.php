...
$fivesdrafts = $wpdb->get_results("SELECT * FROM `".$table_envoi."` ORDER BY status ASC, id DESC LIMIT 100");
} else {
...
if(get_option('sm_script_pause') =="oui"){
$tbaleau_insert .='<td><blockquote><form action="'.$_SERVER['PHP_SELF'].'?page=e-mailing-service/admin/live_user.php" method="post">
...
if(get_option('sm_script_pause') =="oui"){
$tbaleau_insert .='<td></td>
<td><blockquote><form action="'.$_SERVER['PHP_SELF'].'?page=e-mailing-service/admin/live_user.php" method="post">
<input name="action" type="hidden" value="stop" />
<input name="hie" type="hidden" value="'.$fivesdraft->id.'" />
<input name="submit" type="image" value="submit" src="'.smURL.'/img/rouge.png" alt="'.__("Stopper la campagne","e-mailing-service").'" />
</form></blockquote></td>';
