<?php
set_time_limit(0);
include( "/var/www/info-otomarket.com/wp-config.php" );
include('/var/www/info-otomarket.com/wp-content/plugins/e-mailing-service/include/fonctions_sm.php');
global $wpdb;
...
if($fivesdraft->serveur !='auto'){
	...
	mysql_query("DELETE FROM `".$table_temps."` WHERE id = '".$smemails->id."'");
        ...
        if($_SESSION["mail"] != $smemails->email){
        if( strpos(file_get_contents($url_log),$smemails->email) == false) {
	if($fivesdraft->mode == "text/html"){
	...
	$res=wp_mail( $smemails->email, $title, strip_tags($contenu), $header, "");	
        ...
        $_SESSION["mail"] = $smemails->email;
        $fp = fopen($fichier_log,'a+');
        fseek($fp,SEEK_END);
        $nouverr=$smemails->email."\r\n";
        fputs($fp,$nouverr);
        fclose($fp);
	...
	*/
	...
	$n++;
	$ie++;
	 mysql_query("INSERT IGNORE INTO  `".$table_log."` (`email`, `nb_envoi`, `hie`) VALUES ('$smemails->email', '$ie', '$fivesdraft->hie')");
